...
max_len=1300
epochs=64
MAX_TRAINING_SEQ_LEN = 2000
chkpt = "docker_agent_logger/app/binary_classifier/"
...
ds = raw_ds.map(compress, num_parallel_calls=tf.data.AUTOTUNE).prefetch(
    tf.data.AUTOTUNE
)
...
model.train_model(ds,epochs=epochs,chkpt=chkpt+"test3")
